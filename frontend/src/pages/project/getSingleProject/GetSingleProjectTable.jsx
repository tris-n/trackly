...
const CustomTableToolBar = (props) => {
	...
	const {numSelected, filterName, onFilterName, prioritySort, setPrioritySort, handleDeleteTickets, handlePrioritySort, openModal} = props
...
const GetSingleProjectTable = () => {
	...
	const handleSelectAllClick = (event) => {
		if (event.target.checked) {
			const newSelected = singleProjectTickets.map((n) => n._id)
	...
	const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - singleProjectTickets.length) : 0

	const filteredTickets = applySortFilter(singleProjectTickets, getComparator(order, orderBy), filterName, prioritySort, statusSort)

	const isNotFound = !filteredTickets.length && !!filterName

	if (!singleProjectTickets || !user) {
		return <Spinner />
	}
				...
				<TableContainer>
					<Table sx={{width: {mobile: "max-content", tablet: "100%"}}}>
						...
						<CustomTableHead 
							...
							rowCount={singleProjectTickets.length}
...
export default GetSingleProjectTable