...
// Material UI
import { Avatar, Box, Button, Checkbox, IconButton, MenuItem, OutlinedInput, Paper, Popover, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TableSortLabel, Toolbar, Tooltip, Typography } from "@mui/material"
...
const CustomTableToolBar = (props) => {
	...
	const {numSelected, filterName, onFilterName, handleDeleteProjects, openModal} = props
...
const GetAllProjectsTable = () => {
	...
	const filteredProjects = applySortFilter(allProjects, getComparator(order, orderBy), filterName, roleSort)
									...
									>
										...
										>
											<Stack direction="row" alignItems="center">
													<Avatar src={`/images/titleLetters/${project.name.slice(0,1).toLowerCase()}.png`} alt="" width="45px" sx={{mr: 1}}/>
													<strong>{project.name}</strong>
												</Stack>
										</TableCell>

										<TableCell align="left" onClick={(event) => handleRowClick(event, project._id)} sx={{whiteSpace: 'nowrap'}}>{dayjs(new Date(project.createdAt).toLocaleString(), 'DD/MM/YYYY, HH:mm:ss').format('YYYY-MM-DD')}</TableCell>
										<TableCell align="left">{project.projectManagerName === 'zzzzz' ? <span style={{color: "red"}}> unassigned </span> : <CustomUserLink to={`/users/${project.projectManagerId}`}>{project.projectManagerName}</CustomUserLink>}</TableCell>
										<TableCell align="left" onClick={(event) => handleRowClick(event, project._id)}>{project.ticketCount}</TableCell>
