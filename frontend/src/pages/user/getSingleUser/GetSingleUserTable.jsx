...
function applySortFilter(array, comparator, query, prioritySort, statusSort, userId) {

	let stabilizedThis = array.filter(ticket => {
		if (prioritySort !== "All" && ticket.priority !== prioritySort) {
			return false;
		}
		if (statusSort === "creatorId" && ticket.creatorId !== userId) {
			return false;
		}
		if (statusSort === "assignedId" && ticket.assignedId !== userId) {
			return false;
		}
		return true;
	})
...
const CustomTableHead = (props) => {
	...
	const { onSelectAllClick, order, orderBy, numSelected, rowCount, onRequestSort, statusSort } = props
...
const CustomTableToolBar = (props) => {
	...
	const {numSelected, filterName, onFilterName, prioritySort, setPrioritySort, handleDeleteTickets, openModal, handlePrioritySort} = props
...
const GetSingleUserTable = () => {
	...
	const handleSelectAllClick = (event) => {
		if (event.target.checked) {
			const newSelected = singleUserTickets.map((n) => n._id)
	...
	const handleDeleteTickets = () => {
		...
		dispatch(deleteTickets(selected))
			.unwrap()
			.then(() => {
				toast.success(`Successfully deleted the tickets.`)
				setSelected([])
				setPage(0)
				dispatch(getAllTickets({query: [
					{queryType: 'singleUser'},
	...
	const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - singleUserTickets.length) : 0

	const filteredTickets = applySortFilter(singleUserTickets, getComparator(order, orderBy), filterName, prioritySort, statusSort, userId)

	const isNotFound = !filteredTickets.length && !!filterName

	if (!singleUserTickets || !user) {
		return <Spinner />
	}
			...
			<CustomCard sx={{maxWidth: `calc(100vw - 32px)`}}>
				...
				<Box>
					<Tabs value={statusSort} onChange={handleStatusSort}>
						...
						<Tab label="Assigned" value="assignedId" />
						<Tab label="Submitted" value="creatorId" />
					</Tabs>
				</Box>
				...
				<TableContainer>
					<Table sx={{width: {mobile: "max-content", tablet: "100%"}}}>
						...
						<CustomTableHead 
							...
							rowCount={singleUserTickets.length}
...
export default GetSingleUserTable